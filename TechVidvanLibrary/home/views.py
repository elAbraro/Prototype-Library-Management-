...
def addstudent(request):
    if request.session.has_key('is_logged'):
        if request.method == 'POST':
            ...
            if not sname or not studentid:
                messages.error(request, 'Please fill in all fields.')
                return render(request, 'addstudent.html') 
            try:
                new_student = AddStudent(user=request.user, sname=sname, studentid=studentid)  
                new_student.save()
...
def addstudentsubmission(request):
    if request.session.has_key('is_logged'):
        if request.method == "POST":
            ...
            if AddStudent.objects.filter(sname=sname).exists():
                messages.error(request, "Student with this name already exists.")
                return render(request, 'addstudent.html')  # Re-render the form with the error message

            # Check if student ID already exists
            if AddStudent.objects.filter(studentid=studentid).exists():
                messages.error(request, "Student ID already exists.")
                return render(request, 'addstudent.html')  # Re-render the form with the error message

            # If student name and ID are both unique, add the new student
            add = AddStudent(user=user1, sname=sname, studentid=studentid)
            add.save()
            messages.success(request, "Student added successfully.")
            return redirect('/addstudent/')  # Redirect after successful addition
